...
void FrameBuffer::generateTestPattern() {
    ...
    for(int ch = 0; ch < TOTAL_CH; ch++) {
        ...
        for(int c = 0; c < 4; c++) {
            ...
            for(int p = 0; p < n; p++) {
                if(c == 0)
                    s_color_data[c][ch][p] = {.green = 0, .red = BRIGHTNESS, .blue = 0};  // Red
                else if(c == 1)
                    s_color_data[c][ch][p] = {.green = BRIGHTNESS, .red = 0, .blue = 0};  // Green
                else if(c == 2)
                    s_color_data[c][ch][p] = {.green = 0, .red = 0, .blue = BRIGHTNESS};  // Blue
                else
                    s_color_data[c][ch][p] = {.green = 0, .red = 0, .blue = 0};  // Black
            }
        }
    }
